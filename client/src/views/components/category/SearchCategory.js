...
axios.get('/api/category/search/' + this.state.name,data)
.then(res => console.log(res.data))

.catch(err => this.setState({ errors: err.response.data }));

  // Redirect to category-data
  this.props.history.push('/search-category-data/' + this.state.name) 
}
  ...
  render() {
    ...
    return (
      ...
      <Header />
        ...
        <Card>
          <CardHeader>
            <strong>Get Category By Name</strong> 
          </CardHeader>
          <CardBody>
            <Form action="" method="post" key="`${name}`" noValidate onSubmit={this.onSubmit}>
              <FormGroup>
                <Label htmlFor="nf-email">Category Name</Label>
                <Input 
                type="text" 
