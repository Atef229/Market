...
onChange(e) {
  ...
  onSubmit = (e) => {
    ...
    axios.get('/api/admin/' + this.state.admin_id,data)
.then(res => console.log(res.data))

.catch(err => this.setState({ errors: err.response.data }));

  // Redirect to admin-data
  this.props.history.push('/get-admin-data-by-id/' + this.state.admin_id);
          ...
          <CardBody>
            <Form action="" method="post" key="`${admin_id}`" noValidate onSubmit={this.onSubmit}>
              <FormGroup>
                ...
                invalid={errors.admin_id}
                valid={this.state.admin_id}
                   />
                   <FormFeedback>{errors.admin_id}</FormFeedback>
              </FormGroup>
            </Form>
          </CardBody>
          <CardFooter>
